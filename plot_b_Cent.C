...
void plotb() //Результаты фитирования множественности с ратио и классами центральности для Глаубера
{
  // gROOT->SetStyle("Pub");
  gROOT->ForceStyle();
  // gStyle->SetErrorX(0);
  ...
  const char *outDir = "/home/isegal/HADES/centrality/RPC/";
  const char *outDirE = "/home/isegal/HADES/centrality/FW/";
...
//  ptB->AddText("N_{hits} > 16, w/o DCA cut");
  ...
  TFile *fileFitGlE = new TFile(Form("%s%s",outDirE,"glauber_qa.root"));
  TFile *fileFINAL = new TFile(Form("%s%s",outDir,"FINAL.root"));
  TFile *fileFINALE = new TFile(Form("%s%s",outDirE,"FINAL.root"));
  TFile *fileIn = new TFile("/home/isegal/MPD/data/UrQMD/BiBi_UrQMDreq25_new.root"); // file with modal data mult vs b

  TH1F *HistData = (TH1F *)fileFitGl->Get("hitsRPC_selected");
  ...
  TH2F *hBvsRefMultFit = (TH2F *)fileFitGl->Get("B_VS_Multiplicity");
  TH2F *hBvsRefMult = (TH2F *)fileFitGlE->Get("B_VS_Multiplicity");
  ...
  TTree *ResultE = (TTree *)fileFINALE->Get("Result");
  if (!Result)
    return;
  Int_t MinBorderE;
  Int_t MaxBorderE;
  ...
  double BmeanH[Nclasses], BsigmaH[Nclasses], BmeanErH[Nclasses], BsigmaErH[Nclasses];
  ...
  for (int cent = 0; cent < Nclasses; cent++)
  {
    ...
    HData[cent] = hBvsRefMult->ProjectionY(Form("%d_hist_Impact", cent), fMinBorderE, fMaxBorderE);
  ...
  double centbinH[Nclasses], centErH[Nclasses];
  
  for (int i = 0; i < 10; i++){
  	centbinH[i]=centbin[i];
  	centErH[i]=centEr[i];
  }
  ...
  TGraphErrors *GrHades = new TGraphErrors(10, centbinH, BmeanH, centErH, BsigmaH);
  GrHades->SetMarkerStyle(MStyleGl[0]);
  GrHades->SetMarkerSize(1.4);
  GrHades->SetMarkerColor(kRed);
  GrHades->SetLineColor(kRed);
  GrHades->SetLineWidth(2.);
  GrHades->SetTitle("");
  GrHades->GetXaxis()->SetTitle("Centrality, %");
  GrHades->GetYaxis()->SetTitle("<b>, fm");
  ...
  TGraphErrors *GrRatioH = RatioGr(GrHades, GrData, 0, 0.85, 100, 1.15);
  GrRatioH->SetTitle("");
  GrRatioH->GetYaxis()->SetTitle("MC-Gl/Model");
  GrRatioH->GetXaxis()->SetTitle("Centrality, %");
  GrRatioH->GetXaxis()->SetTitleSize(0.1);
  GrRatioH->GetXaxis()->SetLabelSize(0.1);
  GrRatioH->GetYaxis()->SetTitleSize(0.1);
  GrRatioH->GetYaxis()->SetLabelSize(0.1);
  GrRatioH->GetYaxis()->SetTitleOffset(0.65);
  GrRatioH->GetXaxis()->SetTitleOffset(1.5);
  GrRatioH->GetYaxis()->SetLabelOffset(0.013);
  GrRatioH->GetXaxis()->SetLabelOffset(0.04);
  GrRatioH->GetXaxis()->SetNdivisions(210);
  GrRatioH->GetYaxis()->SetNdivisions(505);
  GrRatioH->GetXaxis()->SetTickLength(0.1);
  GrRatioH->GetYaxis()->CenterTitle(true);
