...
#SBATCH --mem=16GB
#SBATCH -J PSD_CENT
#SBATCH -o /lustre/home/user/i/isegal/na61/centrality/log/%A_%a.log

export JOB_ID=$SLURM_ARRAY_JOB_ID
export TASK_ID=$SLURM_ARRAY_TASK_ID

export CentralityFramework=/lustre/home/user/i/isegal/soft/CentralityFramework/centrality/
export kMinPre=$((${TASK_ID}/10))
export kMinPost=$((${TASK_ID}%10-1))
export kMaxPre=$((${TASK_ID}/10))
export kMaxPost=$((${TASK_ID}%10))
export kMin=${kMinPre}.${kMinPost}
export kMax=${kMaxPre}.${kMaxPost}
[ $kMinPost == -1 ] && export kMinPre=$((${TASK_ID}/10-1))
[ $kMinPost == -1 ] && export kMin=${kMinPre}.9
export kStep=.01
export OutDirName=/lustre/home/user/i/isegal/na61/centrality/PbPb13AGeV/psd_mult/
export OutFileIDName=${TASK_ID}
export ConfigName=config_mult.txt
...
scp /lustre/home/user/i/isegal/soft/scripts/submit_cent_spec/${ConfigName} ${jobDir}
scp $CentralityFramework/build/glauber ${jobDir}
sed -i -- "s~Seed: 0~Seed: ${seed}~g" ${jobDir}/${ConfigName}
sed -i -- "s~kMin: 0.0~kMin: ${kMin}~g" ${jobDir}/${ConfigName}
sed -i -- "s~kMax: 0.0~kMax: ${kMax}~g" ${jobDir}/${ConfigName}
...
remove_logs=1
[ $remove_logs == 1 ] && rm /lustre/home/user/i/isegal/na61/centrality/log/*${JOB_ID}*
